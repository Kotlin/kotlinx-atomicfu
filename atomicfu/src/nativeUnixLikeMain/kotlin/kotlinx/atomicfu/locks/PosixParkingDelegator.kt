...
internal actual object ParkingDelegator {
    ...
    actual inline fun timedWait(ref: ParkingData, nanos: Long, shouldWait: () -> Boolean): Unit = memScoped {
        ...
        ts.pointed.tv_nsec = (ts.pointed.tv_nsec.toInt() + nanos % 1_000_000_000).convert()
        //Fix overflow
        if (ts.pointed.tv_nsec.toInt() >= 1_000_000_000) {
            ts.pointed.tv_sec = ts.pointed.tv_sec.toInt().addNanosToSeconds(1_000_000_000).convert()
            ts.pointed.tv_nsec = (ts.pointed.tv_nsec.toLong() - 1_000_000_000).convert()
        }
        callAndVerify { pthread_mutex_lock(ref.mut) }
        try {
